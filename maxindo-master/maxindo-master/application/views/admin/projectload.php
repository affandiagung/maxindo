...
<div class="card card-custom gutter-b example example-compact">
	<div class="card-header">
		<h3 class="card-title"><i class="fa fa-list me-5"></i>Kalender</h3>
		<div class="card-toolbar">
			<div class="example-tools justify-content-center">
				<div class="btn-no-group">
					<a title="Segarkan" role="button" href="javascript:loadcontent('main-content','<?php echo site_url(); ?>/admin/projectload/');" class="btn  btn-light-primary btn-browse me-2"><i class="fa fa-sync-alt"></i> Segarkan</a>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body pt-0 pb-20 px-lg-17">
				...
				<div class="d-flex align-items-center mb-9">
					...
					<div class="fs-6">
						<span class="fw-bolder">Bongkar</span>
			...
			<div class="modal-footer border-0 justify-content-end">
				<button type="button" class="btn btn-success" data-kt-calendar="event_id"><i class="fas fa-list"></i> Detail Project</button>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
	...
	var KTAppCalendar = function() {
		var e, t, n, a, o, r, i, l, d, s, c, m, u, v, f, p, y, D, _, b, k, g, S, Y, h, T, M, w, E, L, id, x = {
			id: "",
			eventName: "",
			eventDescription: "",
			eventLocation: "",
		...
		const q = e => {
			C();
			const n = x.allDay ? moment(x.startDate).format("D MMMM, YYYY") : moment(x.startDate).format("D MMMM, YYYY - HH:mm"),
			a = x.allDay ? moment(x.endDate).format("D MMMM, YYYY") : moment(x.endDate).format("D MMMM, YYYY - HH:mm");
			var o = {
				container: "body",
				trigger: "manual",
				boundary: "window",
				...
				content: '<div class="fw-bolder mb-2">' + x.eventName + '</div><div class="fs-7"><span class="fw-bold">Start:</span> ' + n + '</div><div class="fs-7 mb-4"><span class="fw-bold">End:</span> ' + a + '</div><div id="kt_calendar_event_view_button" type="button" class="btn btn-sm btn-light-primary">Selengkapnya</div>'
			};
		...
		}, // New Event (enggak guna)
		A = () => { // Click
			var e, t, n;
			w.show(), x.allDay ? (e = "All Day", t = moment(x.startDate).format("D MMMM, YYYY"), n = moment(x.endDate).format("D MMMM, YYYY")) : (e = "", t = moment(x.startDate).format("D MMMM YYYY - HH:mm"), n = moment(x.endDate).format("D MMMM YYYY - HH:mm")), g.innerText = x.eventName, S.innerText = e, Y.innerText = x.eventDescription ? x.eventDescription : "--", h.innerText = x.eventLocation ? x.eventLocation : "--", T.innerText = t, M.innerText = n, $(id).attr("data-id", x.id);
			id.addEventListener("click", (e => {
				loadpopup( site_url + "/admin/project_detail/index/pk/PROJECTID/valpk/" + x.id);
			}))
		},
		H = () => {
		...
		return {
			init: function() {
				...
				w = new bootstrap.Modal(B), g = B.querySelector('[data-kt-calendar="event_name"]'), S = B.querySelector('[data-kt-calendar="all_day"]'), Y = B.querySelector('[data-kt-calendar="event_description"]'), h = B.querySelector('[data-kt-calendar="event_location"]'), id = B.querySelector('[data-kt-calendar="event_id"]'), T = B.querySelector('[data-kt-calendar="event_start_date"]'), M = B.querySelector('[data-kt-calendar="event_end_date"]'), E = B.querySelector("#kt_modal_view_event_edit"), L = B.querySelector("#kt_modal_view_event_delete"), F = document.getElementById("kt_calendar_app"), O = moment().startOf("day"), I = O.format("YYYY-MM"), R = O.clone().subtract(1, "day").format("YYYY-MM-DD"), G = O.format("YYYY-MM-DD"), K = O.clone().add(1, "day").format("YYYY-MM-DD"), (e = new FullCalendar.Calendar(F, {
					...
					editable: !0,
					dayMaxEvents: !0,
					events: <?php echo $jsonProjectLoad; ?>, // Load Data Event dari query
					datesSet: function() {
						C()
					}
