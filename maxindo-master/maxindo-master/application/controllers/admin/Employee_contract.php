...
class Employee_contract extends CI_Controller{
	...
	function index(){
		if( isset($this->urisegments['valpk']) ){
		      $this->session->set_userdata('employee_contract_where', $this->uri->uri_to_assoc(4) );
		}
		if( !$this->session->has_userdata("employee_contract_where") ){
	...
	function getparams(){
		...
		$this->params['name'] = $this->lang->line("employee_contract");
		$this->params['maincontent'] = "employee_contract";
		$this->params['simpleform'] = true;
		$this->params['table'] = "employeecontracts";
		$this->params['sql'] = "SELECT EMPLOYEECONTRACTID,employees.NAME EMPLOYEE, employeecontracts.CONTRACTSTARTDATE, employeecontracts.CONTRACTENDDATE, FILE, employeecontracts.DESCRIPTION, CONTRACT FROM employeecontracts
		LEFT JOIN employees ON EMPLOYEEID = EMPLOYEE
		WHERE 1 
		";
		$this->params['order'] = "EMPLOYEECONTRACTID DESC";
	...
	function getData(){
		$where = $this->session->userdata()['employee_contract_where']['pk']." = ".$this->session->userdata()['employee_contract_where']['valpk'];
	...
	function browse(){
		$this->params['alert'] = array(
			'type' => "primary",
			'message' => "Employee : " . $this->Mmasterdata->getEmployeeNameById($this->session->userdata()['employee_contract_where']['valpk'])
		);
		$this->load->library("Engine",$this->params);
	...
	function add(){
		...
		$this->getfieldedit();
		if (count($_POST)>0){
			$_POST['EMPLOYEE'] = $this->session->userdata()['employee_contract_where']['valpk'];
		}
		$this->load->library("Engine",$this->params);
